...
build-shell:
	build -a X64 -p ShellPkg/ShellPkg.dsc -n $(NPROC)
	find Build/ -name '*.efi'
...
sct-image:
	...
	rm -f sct-x64.part1
	/sbin/mkfs.vfat -C sct-x64.part1 131071
	sudo mount sct-x64.part1 mnt -o uid=$(UID)
	...
	cp startup.nsh mnt/
	touch mnt/run
	cp Build/UefiSct/RELEASE_GCC5/SctPackageX64/X64/* mnt/ -R
	cp Build/Shell/RELEASE_GCC5/X64/ShellPkg/Application/Shell/Shell/OUTPUT/Shell.efi mnt/
	mkdir -p mnt/Sequence
	cp uboot.seq mnt/Sequence/
	sudo umount mnt || true
	dd if=/dev/zero of=sct-x64.img bs=1024 count=1 seek=1023
	cat sct-x64.part1 >> sct-x64.img
	rm sct-x64.part1 efi_shell.txt
	echo -e "image1: start=2048, type=ef\n" | \
	/sbin/sfdisk sct-x64.img
